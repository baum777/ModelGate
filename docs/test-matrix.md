...
| S6 | Stream abort is visible as a user-facing error or cancellation | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Browser harness covers abort/cancel and stable recovery; owner: web |
...
| Test ID | Description | Current status | Verification method | Notes / owner |
| --- | --- | --- | --- | --- |
| U1 | App shell renders | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Browser harness now covers the shell render; owner: web |
| U2 | Chat tab opens and is visible | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Tab switching is covered with mocked backend state; owner: web |
| U3 | Matrix Workspace tab opens and is visible | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Tab switching is covered with mocked backend state; owner: web |
| U4 | Header shows health, model alias, and status | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Header truth is verified against mocked backend responses; owner: web |
...
| U6 | Reload restores chat draft and active tab state | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Chat reload smoke verifies the persisted composer state and tab return; owner: web |
...
| U8 | UI shows only the public alias, not provider IDs | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Provider IDs are excluded from visible UI text; owner: web + backend |
| U9 | Beginner navigation exposes only Chat, GitHub Workspace, Matrix Workspace, Review, Settings | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Shell IA uses the beginner-safe nav contract; owner: web |
| U10 | Beginner Mode hides technical identifiers and raw details | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Guarded through `Technische Details`; owner: web |
| U11 | Expert Mode reveals technical details inside labeled disclosure blocks | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Expert disclosure path stays explicit; owner: web |
| U12 | Browser does not call `api.github.com` or `matrix.org` directly | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | MosaicStack backend remains the only browser authority; owner: web + backend |

## 7. Chat UI Inputs

| Test ID | Description | Current status | Verification method | Notes / owner |
| --- | --- | --- | --- | --- |
| UI-C1 | Typing updates the composer | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Composer input is verified in browser; owner: web |
| UI-C2 | Send button submits the prompt | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Submit path is exercised through mocked SSE; owner: web |
| UI-C3 | `Cmd/Ctrl+Enter` submits the prompt | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Keyboard shortcut is exercised in browser; owner: web |
| UI-C4 | Empty send is blocked | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Disabled-submit guard is verified; owner: web |
| UI-C5 | Send is disabled while streaming | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Deterministic browser harness checks the disabled state during an in-flight chat request; owner: web |
| UI-C6 | Backend errors surface as an error banner | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Mocked `POST /chat` error responses render visibly; owner: web |
...
| UI-C10 | Stream error does not create a fake final assistant message | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Browser harness verifies only the user message is finalized on error; owner: web |
...
| Test ID | Description | Current status | Verification method | Notes / owner |
| --- | --- | --- | --- | --- |
| E1 | Opening the Matrix tab starts bootstrap | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Matrix bootstrap is exercised via mocked read-only backend state; owner: web |
| E2 | Successful `whoami` shows identity | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Identity text is rendered from mocked backend state; owner: web + backend |
| E3 | Successful joined-rooms load shows list or grid | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Joined rooms render from mocked backend state; owner: web + backend |
...
| E11 | Backend errors surface stage-specific messages | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Fail-closed Matrix error state is exercised in browser; owner: web |
...
| E13 | Provenance loads from the backend route after scope resolution | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Browser calls the encoded provenance route and renders the returned read-only metadata; owner: web + backend |
...
| T7 | UI shows verified result | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Room topic review shows verified backend readback; owner: web |
...
| T13 | Refresh plan reloads canonical topic state | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Refresh uses only `GET /api/matrix/actions/:planId` and re-renders canonical plan fields; owner: web |
| T14 | Expired or missing plan refresh fails closed | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Refresh clears executable state and blocks approval/execution on backend error; owner: web |
...
| A2 | Provenance markers are backend-issued only | automated | [server/test/matrix-routes.test.ts](../server/test/matrix-routes.test.ts) and [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Read-only provenance is backend-owned, derived, and browser-rendered; owner: backend + web |
...
| Test ID | Description | Current status | Verification method | Notes / owner |
| --- | --- | --- | --- | --- |
| N1 | Provider IDs do not appear in the UI | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Visible DOM text excludes provider IDs; owner: web |
| N2 | Matrix token does not appear in the browser | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Visible DOM text excludes Matrix and OpenRouter secrets; owner: web + backend |
...
| N8 | Stale plan execution is blocked | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) and [server/test/matrix-actions.test.ts](../server/test/matrix-actions.test.ts) | Browser UI fails closed on stale Matrix plans and the backend rejects stale writes; owner: backend + web |
...
| A12 | Composer remains focusable after send | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Browser harness confirms the composer can be refocused after submit; owner: web |
| A13 | Composer remains focusable after error | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Browser harness confirms the composer can be refocused after a backend error; owner: web |
| A14 | Live region is present during streaming | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Chat message list exposes an `aria-live="polite"` region; owner: web |
...
| A16 | Buttons have accessible labels | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Browser harness queries primary controls by accessible name; owner: web |
| A17 | Status is not color-only | automated | [tests/browser/mosaicstack.spec.ts](../tests/browser/mosaicstack.spec.ts) | Status pills and alerts expose explicit text, not just color; owner: web |
